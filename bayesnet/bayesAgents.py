...
def constructBayesNet(gameState):
    ...
    def append(edges, X_POS_VAR, Y_POS_VAR, FOOD_HOUSE_VAR, GHOST_HOUSE_VAR):
    ...
    # set each variableDominsDict[var] = set of possible assignments to var
    def setAssignments():
    ...
    # populate obsVars with given procedure
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            variableDomainsDict[observation] = [BLUE_OBS_VAL, RED_OBS_VAL, NO_OBS_VAL]

    append(edges, X_POS_VAR, Y_POS_VAR, FOOD_HOUSE_VAR, GHOST_HOUSE_VAR)
    setAssignments()

    variableList = [X_POS_VAR, Y_POS_VAR] + obsVars + HOUSE_VARS
    edgeTuplesList = edges

    bayesNet = bn.constructEmptyBayesNet(variableList, edgeTuplesList, variableDomainsDict)

    return bayesNet, obsVars
...
def fillObsCPT(bayesNet, gameState):
    ...
    def setPosition(obsPos):
        if abs(obsPos[0] - topRightPos[0]) < upperBound and abs(obsPos[1] - topRightPos[1]) < upperBound:
            position = TOP_RIGHT_VAL
        if abs(obsPos[0] - topLeftPos[0]) < upperBound and abs(obsPos[1] - topLeftPos[1]) < upperBound:
            position = TOP_LEFT_VAL
        if abs(obsPos[0] - bottomRightPos[0]) < upperBound and abs(obsPos[1] - bottomRightPos[1]) < upperBound:
            position = BOTTOM_RIGHT_VAL
        if abs(obsPos[0] - bottomLeftPos[0]) < upperBound and (abs(obsPos[1] - bottomLeftPos[1]) < upperBound):
            position = BOTTOM_LEFT_VAL
        return position

    def setFoodHouseProb(probability=0.0):
        if assign[observations] == RED_OBS_VAL:
            probability = PROB_FOOD_RED
        if assign[observations] == BLUE_OBS_VAL:
            probability = 1.0 - PROB_FOOD_RED
        return probability

    def setGhostHouseProb(probability=0.0):
        if assign[observations] == RED_OBS_VAL:
            probability = PROB_GHOST_RED
        if assign[observations] == BLUE_OBS_VAL:
            probability = 1.0 - PROB_GHOST_RED
        return probability

    def setNoObsProb(probability=0.0):
        if assign[observations] == NO_OBS_VAL:
            probability = 1.0
        return probability

    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            observations = OBS_VAR_TEMPLATE % obsPos
            obsFactor = bn.Factor([observations], [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], bayesNet.variableDomainsDict())

            position = setPosition(obsPos)

            for assign in obsFactor.getAllPossibleAssignmentDicts():
                probability = defaultProbability
                if assign[FOOD_HOUSE_VAR] == position:
                    probability = setFoodHouseProb()
                elif assign[GHOST_HOUSE_VAR] == position:
                    probability = setGhostHouseProb()
                else:
                    probability = setNoObsProb()

                obsFactor.setProbability(assign, probability)

            bayesNet.setCPT(observations, obsFactor)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    for assignment in assignments:
        ...
        if newProb > probability:
            probability = max(newProb, probability)
            probableLocation = assignment
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        # util.raiseNotDefined()
        leftHouse = {}
        leftHouse.update(evidence)
        rightHouse = {}
        rightHouse.update(evidence)

        leftHouse[FOOD_HOUSE_VAR] = TOP_LEFT_VAL
        leftHouse[GHOST_HOUSE_VAR] = TOP_RIGHT_VAL
        rightHouse[FOOD_HOUSE_VAR] = TOP_RIGHT_VAL
        rightHouse[GHOST_HOUSE_VAR] = TOP_LEFT_VAL

        queryVariables = [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR]
        factor = inference.inferenceByVariableElimination(self.bayesNet, queryVariables, evidence, eliminationOrder)

        # set the left and right expected values
        leftExpectedValue = factor.getProbability(leftHouse) * WON_GAME_REWARD + factor.getProbability(rightHouse) * GHOST_COLLISION_REWARD
        rightExpectedValue = factor.getProbability(leftHouse) * GHOST_COLLISION_REWARD + factor.getProbability(rightHouse) * WON_GAME_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for prob, explorationEvidence in self.getExplorationProbsAndOutcomes(evidence):
            leftValue, rightValue = self.computeEnterValues(explorationEvidence, enterEliminationOrder)
            expectedValue = expectedValue + prob * max(leftValue, rightValue)
