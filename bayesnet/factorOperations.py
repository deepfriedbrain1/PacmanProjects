...
def joinFactors(factors):
    ...
    for assign in assignments:
        ...
        for factor in factors:
            prob = product(joinedFactor.getProbability(assign), factor.getProbability(assign))
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor, eliminationVariable):
        ...
        return getNewFactor(assignments, newFactor, factor)
...
def normalize(factor):
    ...
    probability = 0.0
    ucv1 = factor.unconditionedVariables()
    ucv2 = {}
    ucv2 = ucv1.copy()
    cv = factor.conditionedVariables()
    unionSet = set()
    ...
    def getProbability(assignments, probability=0.0):
        for assign in assignments:
            probabilityFactor = factor.getProbability(assign)
            probability = probabilityFactor + probability
        return probability

    # return the new factor
    def getNewFactor(assignments, newFactor, factor, probability):
        for assign in assignments:
            prob = factor.getProbability(assign)
            newProb = prob / probability
            newFactor.setProbability(assign, newProb)
        return newFactor

    # add the unconditioned variable to a set
    for variable in ucv1:
        unionSet.add(variable)
    # add the conditioned variable to a set
    for variable in cv:
        unionSet.add(variable)

    for uc in ucv1:
        vd = factor.variableDomainsDict()[uc]
        if len(vd) == 1:
            ucv2.remove(uc)
            cv.add(uc)

    probability = getProbability(assignments)
    assignments = factor.getAllPossibleAssignmentDicts()
    newFactor = Factor(ucv2, cv, variableDomainsDict)

    return getNewFactor(assignments, newFactor, factor, probability)
