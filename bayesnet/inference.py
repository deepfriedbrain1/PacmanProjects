...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        # set up the conditional probability tables
        cpt = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        lowerBound = 2

        for variable in eliminationOrder:
            cpt, joinedFactor = joinFactorsByVariable(cpt, variable)

            if len(joinedFactor.unconditionedVariables()) >= lowerBound:
                cpt.append(eliminate(joinedFactor, variable))
